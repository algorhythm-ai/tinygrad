    ...
    if uop == UOps.IF:
      ...
      if uop == UOps.LOOP:
        ...
        if args[0] == "METAL":
          ...
          kk("float wmma0[8];")
          kk("{")
          kk("wmma::fragment<wmma::matrix_a, 16, 16, 16, half, wmma::row_major> a_frag;")
          kk("wmma::fragment<wmma::matrix_b, 16, 16, 16, half, wmma::row_major> b_frag;")
          kk("wmma::fragment<wmma::accumulator, 16, 16, 16, float> c_frag;")
          kk("wmma::fill_fragment(c_frag, 0.0f);")
          kk("__shared__ float c[16*16];")
          kk("wmma::load_matrix_sync(a_frag, data1, 16);")
          kk("wmma::load_matrix_sync(b_frag, data2, 16);")
          kk("wmma::mma_sync(c_frag, a_frag, b_frag, c_frag);")
          kk("wmma::store_matrix_sync(c, c_frag, 16, wmma::mem_row_major);")
          for i in range(8):
